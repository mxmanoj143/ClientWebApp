  ...
  if (done) {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-8 text-center" data-testid="contact-success">
        ...
        <div className="font-display font-black text-2xl text-[#0F172A] mt-4">Message Sent</div>
        <p className="text-[#475569] mt-2">Thanks for reaching out. We will get back to you within one business day.</p>
        <button onClick={() => setDone(false)} className="mt-6 text-sm font-bold text-[#EA580C]" data-testid="contact-send-another">Send another →</button>
      </motion.div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="glass rounded-2xl p-6 md:p-8 space-y-5" data-testid="contact-form">
      ...
      <button
        ...
        className="inline-flex items-center justify-center gap-2 bg-[#0F172A] hover:bg-[#EA580C] text-white px-8 py-4 font-bold tracking-wide rounded-full disabled:opacity-60 transition-colors"
      >
