...
export default function Footer() {
  return (
    <footer data-testid="site-footer" className="bg-[#0F172A] text-white relative noise-overlay">
      <div className="px-6 md:px-12 lg:px-24 py-20">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          <div className="md:col-span-5">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-[#EA580C] flex items-center justify-center rounded-md">
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs uppercase tracking-[0.25em] text-[#EA580C] font-bold">Concrete Products</div>
              </div>
            </div>
            <p className="mt-6 text-sm text-white/70 max-w-md leading-relaxed">{COMPANY.description}</p>
            <div className="mt-6 text-xs uppercase tracking-[0.2em] text-[#EA580C] font-bold">{COMPANY.tagline}</div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <ul className="space-y-2">
              {NAV_LINKS.map((l) => (
                <li key={l.to}>
                  <Link to={l.to} className="text-sm hover:text-[#EA580C] transition-colors" data-testid={`footer-link-${l.label.toLowerCase()}`}>
                    {l.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="md:col-span-2">
            ...
            <ul className="space-y-2">
              {PRODUCTS.slice(0, 8).map((p) => (
                <li key={p.id}>
                  <Link to="/products" className="text-sm text-white/80 hover:text-[#EA580C] transition-colors">
                    {p.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="md:col-span-3">
            ...
            <Link to="/contact" data-testid="footer-cta-quote" className="mt-6 inline-flex items-center gap-2 bg-[#EA580C] hover:bg-[#C2410C] px-5 py-3 text-sm font-bold rounded-full transition-colors">
