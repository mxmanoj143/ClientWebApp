            ...
            <Reveal key={s.id} delay={i * 0.05}>
              <div className={`grid lg:grid-cols-12 gap-10 items-center ${i % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`} data-testid={`service-${s.id}`}>
                <div className="lg:col-span-5">
                  <div className="aspect-[4/5] bg-[#0F172A] rounded-3xl relative overflow-hidden noise-overlay">
                    <img
                    ...
                    <div className="absolute bottom-6 left-6 w-14 h-14 bg-[#EA580C] rounded-2xl flex items-center justify-center">
                      <Ico className="w-6 h-6 text-white" />
                    </div>
                  </div>
                </div>
                <div className="lg:col-span-7">
                  <div className="text-xs uppercase tracking-[0.3em] text-[#EA580C] font-bold">0{i + 1} — Service</div>
                  <h2 className="font-display font-black text-4xl md:text-5xl text-[#0F172A] mt-4 leading-[1.05]">{s.title}</h2>
                  <p className="text-[#475569] mt-6 leading-relaxed">{s.desc}</p>
                  <ul className="mt-8 grid sm:grid-cols-2 gap-3">
                    {s.points.map((p) => (
                      <li key={p} className="flex items-start gap-3 text-[#0F172A]">
                        <CheckCircle2 className="w-5 h-5 text-[#EA580C] flex-shrink-0 mt-0.5" />
                        <span className="text-sm font-medium">{p}</span>
                      </li>
                    ))}
                  </ul>
                  <Link to="/contact" className="mt-10 inline-flex items-center gap-2 bg-[#0F172A] text-white px-6 py-3 font-bold rounded-full hover:bg-[#EA580C] transition-colors text-sm" data-testid={`service-cta-${s.id}`}>
